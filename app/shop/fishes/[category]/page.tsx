...
import { ShoppingCart, ArrowLeft, Eye, CheckCircle } from "lucide-react";
import { motion } from "framer-motion";

export default function FishListingPage({ params }: { params: Promise<{ category: string }> }) {
  const { category } = use(params);
  const products = getProductsByCategory("fish", category);
    ...
    <>
      ...
      <main className="min-h-screen bg-bg pt-28 pb-16 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          ...
          <Link
            href="/shop/fishes"
            className="mb-8 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-text-secondary hover:text-cyan-505 dark:hover:text-cyan-400 transition-colors duration-300"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Categories
          </Link>

          <div className="mb-12">
            <span className="text-xs font-semibold tracking-widest uppercase text-cyan-500 dark:text-cyan-400">Specimen Catalog</span>
            ...
            <p className="text-text-secondary text-xs md:text-sm mt-3 max-w-xl leading-relaxed">
              Premium quality aquatic life acclimated and certified for high-end setups. Scroll to discover available specimens.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product, index) => (
                <motion.div
                  ...
                  className="group luxury-card rounded-2xl overflow-hidden flex flex-col h-full bg-card border border-border hover:border-cyan-500/20"
                >
                  ...
                  <div className="relative h-64 w-full overflow-hidden border-b border-border bg-card-secondary">
                    ...
                    <div className="absolute top-4 left-4">
                      {(product.stock ?? 0) > 0 ? (
                        <span className="px-2.5 py-1 rounded-full text-[9px] font-bold tracking-widest uppercase bg-black/60 backdrop-blur-md text-emerald-455 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                  ...
                  <div className="p-6 flex flex-col flex-1 justify-between gap-5">
                    <div>
                      <div className="flex justify-between items-baseline mb-1">
                        <h3 className="text-lg font-bold text-text group-hover:text-cyan-505 dark:group-hover:text-cyan-400 transition-colors truncate">
                          {product.name}
                        </h3>
                        <span className="text-base font-black text-cyan-505 dark:text-cyan-400">${product.price}</span>
                      </div>
                    ...
                    <div className="pt-4 border-t border-border flex flex-col gap-2.5">
                      <div className="flex items-center gap-2 text-[10px] font-semibold text-text-secondary mb-1">
                        <CheckCircle className="w-3.5 h-3.5 text-cyan-500 shrink-0" />
                        <span>Tank Rec: {product.tankSize || "N/A"}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-center text-xs font-bold uppercase tracking-wider text-white shadow-[0_0_10px_rgba(34,211,238,0.1)] hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100 disabled:shadow-none cursor-pointer"
                        >
